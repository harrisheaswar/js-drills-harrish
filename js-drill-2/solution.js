...
export function groupUsersByProgLang(){
    ...
    for(let i=0;i<sample.length;i++){                                                                   // Outer loop runs each regex 

        let arr=[];

            for(let name in users){

                if(users[name].hasOwnProperty("designation") && sample[i].test(users[name].designation)){     // matching the regex to every element of user
